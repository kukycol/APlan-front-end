<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-input v-model="listQuery.paramName" placeholder="参数名称" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
      <el-input v-model="listQuery.paramKey" placeholder="参数键名" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
      <el-select v-model="listQuery.isSystem" placeholder="选择评分" clearable style="width: 110px" class="filter-item">
    ...
    >
      ...
      <el-table-column label="评分条件" align="center">
        <template slot-scope="{row}">
          <span>{{ row.scoreContent }}</span>
        </template>
      </el-table-column>
      <el-table-column label="评分类型" align="center">
        <template slot-scope="{row}">
          <span>{{ row.scoreType }}</span>
        </template>
      </el-table-column>
      <el-table-column label="评分条件最大分数" align="center">
        <template slot-scope="{row}">
          <span>{{ row.scoreMax }}</span>
        </template>
      </el-table-column>
      <el-table-column v-if="isAuth('sys:role:update')" label="Actions" align="center" class-name="small-padding fixed-width">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="left" label-width="140px" style="width: 400px; margin-left:50px;">
        <el-form-item label="评分条件" prop="scoreContent">
          <el-input v-model="temp.scoreContent" />
        </el-form-item>
        <el-form-item label="评分条件最大分数" prop="scoreContent">
          <el-input-number v-model="temp.scoreMax" :min="1" :max="100"  controls-position="right"/>
        </el-form-item>
        <el-form-item label="评分类型" prop="scoreType">
          <el-radio-group v-model="temp.scoreType" size="small">
            <el-radio-button :label="0">星级</el-radio-button>
...
<script>
import { pageScore, delScore, btcScore, createScore, updateScore } from '@/api/score-table'
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      pageScore(this.listQuery).then(response => {
    ...
    btcDelete() {
      ...
      for (const ddElement of dd) {
        ids += ',' + ddElement.scoreId
      }
      ids = ids.substring(1)
      btcScore({ 'ids': ids }).then((res) => {
        if (res.code === 20000) {
          this.$notify({
            title: 'Success',
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createScore(this.temp).then((res) => {
            if (res.code === 20000) {
              // 显示表单提示按钮
    ...
    handleDelete(row, index) {
      delScore({ 'ids': row.scoreId }).then((res) => {
        console.log(res)
        this.$notify({
          title: 'Success',
          message: 'Delete Successfully',
